...
namespace eg
{
    namespace dx
    {
        inline std::string parse(HRESULT result)
        {
            if (FAILED(result))
            {
                ...
                std::string errorTxt = boost::str(boost::format("Direct3D call failed. Error '%1%', Description '%2%'") % error % desription);
                return  errorTxt;
            }
            return "";
        }
    ...
    #define HR_REPORT_INVALID(result) {if (FAILED(result)) {log_error << eg::dx::parse(result);}}
